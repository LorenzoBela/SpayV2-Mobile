...
const getMilestoneRarity = (id: string): Rarity => {
  ...
  if (
    ...
    id.includes('limit-raise-250k') ||
    id.includes('500-orders')
  ) {
    return 'EPIC';
  }
  if (
    ...
    id.includes('limit-raise-100k') ||
    id.includes('hundred-clients') ||
    id.includes('fifty-clients')
  ) {
    return 'RARE';
  }
  if (
    ...
    id.includes('twenty-five-orders') ||
    id.includes('single-20k') ||
    id.includes('single-10k') ||
    id.includes('twenty-clients') ||
    id.includes('double-digit-clients')
  ) {
    return 'UNCOMMON';
  }
  return 'COMMON';
...
export default function AdminMilestonesScreen() {
  ...
  // Fetch admin milestones data using tRPC
  ...
  const tabs = [
    ...
    { key: 'clients', label: 'Clients Base' },
  ];
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: t.bg }]} edges={['top', 'left', 'right']}>
      ...
      {isLoading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: t.textSecondary }]}>Loading global milestones...</Text>
        </View>
